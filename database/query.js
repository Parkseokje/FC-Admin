...
QUERY.ADMIN = {
  ...
  GetList: "SELECT * FROM `admin` " + "WHERE fc_id= ? " + "ORDER BY `id` ASC; ",
...
QUERY.EDU = {
  ...
  InsertIntoLogGroupUser:
    "INSERT INTO `log_group_user` (`user_id`, `group_id`) " + "VALUES(?,?); ",
...
QUERY.DASHBOARD = {
  GetUserCount:
    "SELECT count(*) AS total_users FROM `users` " + "WHERE fc_id= ?; ",
  GetBranchCount:
    "SELECT count(*) total_branch FROM `branch` " + "WHERE fc_id= ?;",
  GetCurrentEduCount:
