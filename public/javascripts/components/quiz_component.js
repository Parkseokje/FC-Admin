    ...
    QuizComponent.prototype = {
            ...
            if (this.data.quiz_id) {
                ...
                switch (this.data.quiz_type) {
                    ...
                    case 'C': // 다답형
                        for (
                            var index = 0;
                            index < this.data.options.length;
                            index++
                        ) {
        ...
        create_option_events: function() {
            if (this.data.quiz_type !== 'A') {
                ...
                var input_add_option = this.el.querySelector(
                    '#input-add-option'
                )
                var btn_remove_option = this.el.querySelector(
                    '.btn_remove_option'
                )
                var option_box = this.el.querySelector('#option-box')

                // 보기 추가 (엔터키 입력시)
                input_add_option.addEventListener(
                    'keypress',
                    function(e) {
                ...
                // 보기 추가 ( + 버튼 클릭시)
                btn_add_option.addEventListener(
                    'click',
                    function(e) {
                        e.preventDefault()
        ...
        validate: function() {
            ...
            var nodes = this.el.querySelectorAll(
                'input[type=text]:not(#input-add-option)'
            )
            var i,
                correct_checked = 0 // 정답체크한 보기의 수
        ...
        create_quiz: function() {
            ...
            // box header
            innerstr +=
                '   <div class="box-header" data-index="' +
                this.options.index +
                '">'
            ...
            // }
            innerstr +=
                '               <button type="button" class="btn btn-default btn-sm btn-remove-quiz" data-widget="remove">'
            ...
            if (this.data.quiz_type === 'A') {
                ...
                innerstr +=
                    '           <input type="text" class="form-control" name="question" id="question" placeholder="질문을 입력하세요" required autocomplete="off" />'
                innerstr += '       </div>'
                innerstr += '       <div class="form-group">'
                innerstr +=
                    '           <input type="text" class="form-control" name="answer_desc" id="answer_desc" placeholder="답변을 입력하세요" required autocomplete="off" />'
                innerstr += '       </div>'
                innerstr += '   </div>'
            } else {
                innerstr += '   <div class="box-body">'
                ...
                innerstr +=
                    '           <input id="input-add-option" class="form-control" placeholder="추가할 보기 내용을 입력하세요">'
                innerstr += '           <div class="input-group-btn">'
                innerstr +=
                    '               <button id="btn-add-option" type="button" class="btn btn-default"><i class="fa fa-plus"></i></button>'
                innerstr += '           </div>'
                innerstr += '       </div>'
                innerstr += '   </div>'
            }
        ...
        create_quiz_options: function(data) {
            ...
            item.onclick = function(e) {
                if (e.target.classList[2] == 'btn-remove-option') {
                    if (data) {
                        var option_id = $(e.target)
                            .closest('.item')
                            .data('option-id')
                        _self.options.parent.deleteQuizOption(
                            option_id,
                            function() {
            ...
            if (data) {
                if (this.data.quiz_type === 'B')
                    innerstr +=
                        ...
                        this.options.index +
                        '" class="correct-checkbox" type="radio" ' +
                        (data.iscorrect === 1 ? 'checked' : '') +
                        ' />'
                else
                    innerstr +=
                ...
                innerstr +=
                    '           <input type="text" class="form-control options" required autocomplete="off" placeholder="보기를 입력하세요" data-option-id="' +
                    data.id +
                    '" value="' +
                    data.option +
                    '" />'
                ...
                innerstr +=
                    '       <div class="input-group" style="width: 100%">'
                innerstr +=
                    '           <input type="text" class="form-control options" required autocomplete="off" placeholder="보기를 입력하세요" value="' +
                    input_add_option.value +
                    '" />'
            }

            innerstr +=
                '           <div class="input-group-btn btn-remove-option">'
            innerstr +=
                '               <button id="btn-remove-option" type="button" class="btn btn-danger btn-remove-option"><i class="fa fa-minus btn-remove-option"></i></button>'
            innerstr += '           </div>'
            innerstr += '       </div>'
            innerstr += '    </div>'
            innerstr += '</div>'
