...
require(["common"], function(Util) {
  ...
  btnCreateVideo.bind("click", function(e) {
    ...
    Util.createWindowPopup(
      "/course/create/video?course_id=" + courseId,
      "Video",
      windowOption
    );
  });
  ...
  btnCreateQuiz.bind("click", function(e) {
    ...
    var courseId = $(this).attr("data-course-id");
    Util.createWindowPopup(
      "/course/create/quiz?course_id=" + courseId + "&type=QUIZ",
      "Quiz",
      windowOption
    );
  });
  ...
  btnCreateFinalQuiz.bind("click", function(e) {
    ...
    var courseId = $(this).attr("data-course-id");
    Util.createWindowPopup(
      "/course/create/quiz?course_id=" + courseId + "&type=FINAL",
      "Final",
      windowOption
    );
  });
  ...
  function deleteSession(params) {
    axios
      ...
      .then(function(response) {
        if (!response.data.success) {
          alert(
            "진행한 이력이 있어 세션을 삭제하지 못했습니다. 관리자에게 문의해주시기 바랍니다."
          );
        } else {
          alert("세션을 삭제하였습니다.");
          location.reload();
