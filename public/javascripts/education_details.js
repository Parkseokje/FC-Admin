...
require(["common"], function(Util) {
  ...
  function makeCourseGroupList() {
    ...
    $("#draggablePanelList")
      ...
      .each(function(index) {
        ...
        if (course_group.id) {
          if ($(this).is(":visible")) course_group.mode = "UPDATE";
          else course_group.mode = "DELETE";
        } else {
  ...
  function addCourseGroupItem() {
    ...
    // 강의 중복추가 방지
    var duplicated_item = $(
      ".list-group-item[data-course-id=\"" + course_id + "\" ]"
    );
    ...
    element +=
      "<li class=\"list-group-item\" data-course-id=\"" +
      course_id +
      "\" data-course-group-id=\"\" data-course-group-key=\"" +
      course_group_key +
      "\">";
  ...
  btnRegisterPointWeight.bind("click", function(e) {
    ...
    if (pointTotal !== 100) {
      alert(
        "포인트의 합계가 100이 되어야 합니다. 설정 값을 다시 확인해 주세요"
      );
      e.preventDefault();
    } else {
