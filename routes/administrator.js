    ...
    async.series(
      [
        callback => {
          connection.query(
            QUERY.ADMIN.GetList,
            [req.user.admin_id, req.user.fc_id],
            (err, results) => {
...
router.get("/branch/:admin_id", util.isAuthenticated, (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(
      QUERY.ADMIN.GetAdminBranch,
      [adminId, req.user.fc_id],
      (err, results) => {
        connection.release();
...
router.post("/modify", util.isAuthenticated, (req, res, next) => {
  ...
  if (userName === "" || userEmail === "" || !util.isValidEmail(userEmail)) {
    return next({
      status: 500,
      message: "잘못된 형식의 이메일이 존재합니다."
    });
...
router.post("/role/reset", (req, res) => {
  ...
  if (role === "" || userId === "") {
    ...
    pool.getConnection((err, connection) => {
      if (err) throw err;
      connection.query(QUERY.ADMIN.ResetRole, [role, userId], (err, result) => {
        connection.release();
...
router.get("/offices", (req, res, next) => {
  ...
  pool.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(
      QUERY.ADMIN.GetOfficeBranchesByOfficeId,
...
router.post("/office/branches", (req, res, next) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(
      QUERY.ADMIN.DeleteOfficeBranches,
      [officeId],
      (err, data) => {
        if (!err) {
          async.whilst(
            ...
            callback => {
              connection.query(
                QUERY.ADMIN.InsertIntoOfficeBranches,
...
router.post("/admin/offices", (req, res, next) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(QUERY.ADMIN.DeleteAdminOffices, [adminId], (err, data) => {
      if (!err) {
        async.whilst(
          () => {
          ...
          callback => {
            connection.query(
              QUERY.ADMIN.InsertIntoAdminOffices,
