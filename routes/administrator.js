...
router.get("/", isAuthenticated, function(req, res) {
  async.series(
    [
      function(callback) {
        connection.query(QUERY.ADMIN.GetList, [req.user.fc_id], function(
          err,
          results
        ) {
      ...
      function(callback) {
        connection.query(QUERY.EMPLOYEE.GetBranch, [req.user.fc_id], function(
          err,
          results
        ) {
          callback(err, results);
...
router.post("/modify", isAuthenticated, function(req, res, next) {
  ...
  if (
    ...
    connection.query(
      ...
      [_data.name, _data.email, _data.id],
      function(err, result) {
        if (err) {
          // console.error(err);
          // res.redirect('/process?url=administrator&msg=error');
...
router.post("/role/reset", function(req, res) {
  ...
  if (_role === "" || _user_id === "") {
    ...
    connection.query(QUERY.ADMIN.ResetRole, [_role, _user_id], function(
      err,
      result
    ) {
      if (err) {
