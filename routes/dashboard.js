    ...
    async.series(
      [
        ...
        callback => {
          startTime = new Date();
          connection.query(
            QUERY.DASHBOARD.GetUserCount(req.user),
            [],
            (err, rows) => {
              if (err) {
                console.error(err);
        ...
        callback => {
          ...
          connection.query(
            QUERY.DASHBOARD.GetBranchCount(req.user),
            [],
            (err, rows) => {
              if (err) {
                console.error(err);
        ...
        callback => {
          ...
          connection.query(
            QUERY.DASHBOARD.GetCurrentEduCount(req.user),
            [],
            (err, rows) => {
              if (err) {
                console.error(err);
        ...
        callback => {
          ...
          connection.query(
            QUERY.DASHBOARD.GetThisMonthProgress(req.user),
            [],
            (err, rows) => {
              endTime = new Date() - startTime;
        ...
        callback => {
          ...
          connection.query(
            QUERY.DASHBOARD.GetThisMonthProgressByEdu(req.user),
            [],
            (err, rows) => {
              endTime = new Date() - startTime;
        ...
        callback => {
          connection.query(
            QUERY.DASHBOARD.GetBranchProgressAll(req.user),
            [],
            (err, rows) => {
              callback(err, rows);
            }
        ...
        callback => {
          ...
          connection.query(
            QUERY.DASHBOARD.GetUserPointList(req.user),
            [],
            (err, rows) => {
              endTime = new Date() - startTime;
      ...
      (err, result) => {
        ...
        if (err) {
          ...
          var branchBottomMost = result[5].slice(0).sort((a, b) => {
    ...
    async.series(
      [
        ...
        callback => {
          // if (pointWeight != null) {
          connection.query(
            QUERY.DASHBOARD.GetUserPointListByEduId(_inputs.edu_id, req.user),
            [],
            (err, rows) => {
              // console.log(q.sql);
              if (err) throw err;
              ...
              for (let index = 0; index < rows.length; index++) {
                if (rows[index].logs !== null) {
                  ...
                  let userPeriod =
                    logs.speed.user_period === null
                      ? 0
                      : logs.speed.user_period;
                  ...
                  rows[index].reeltime =
                    (logs.reeltime.video_watch_count === undefined
                      ? "0"
                      : logs.reeltime.video_watch_count) +
                    " / " +
                    (logs.reeltime.video_count === undefined
                      ? "0"
                      : logs.reeltime.video_count);
                  // rows[index].reeltime = logs.reeltime.played_seconds + ' / ' + logs.reeltime.duration;
                  rows[index].speed =
                    userPeriod + " / " + logs.speed.edu_period;
