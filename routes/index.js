...
// 엑셀 파일 업로드하기
router.post("/upload/excel/create/employee_x", isAuthenticated, function(
  req,
  res,
  next
) {
  ...
  async.waterfall(
    [
      ...
      function(data, callback) {
        RegisterUserService.createUser(data, req.user.fc_id, function(
          err,
          ret
        ) {
          // console.info('C');
 ...
 * 직원목록 엑셀 업로드
 */
router.post("/upload/excel/create/employee", isAuthenticated, function(
  req,
  res,
  next
) {
  ...
  async.waterfall(
    [
      ...
      function(file_path, callback) {
        ...
        wb.xlsx.readFile(file_path).then(function() {
              ...
              result.push({
                ...
                branch:
                  row.values[1] == undefined
                    ? ""
                    : UTIL.replaceEmptySpace(row.values[1]),
                duty:
                  row.values[2] == undefined
                    ? ""
                    : UTIL.replaceEmptySpace(row.values[2]),
                name:
                  row.values[3] == undefined
                    ? ""
                    : UTIL.replaceEmptySpace(row.values[3]),
                phone: UTIL.getDigitOnly(
                  row.values[4] == undefined
                    ? ""
                    : UTIL.replaceEmptySpace(row.values[4])
                ),
                email:
                  row.values[5] == undefined
                    ? ""
                    : UTIL.replaceEmptySpace(row.values[5]),
                error: false,
                error_msg: []
              });
      ...
      function(excel_data_2, callback) {
        ...
        connection.query(QUERY.EDU.GetUserDataByPhone, [phone], function(
          err,
          data
        ) {
          if (data.length > 0) {
            for (index = 0, len = data.length; index < len; index++) {
              for (
                index2 = 0, len2 = excel_data_2.length;
                index2 < len2;
                index2++
              ) {
      ...
      function(excel_data_3, callback) {
        ...
        connection.query(QUERY.EDU.GetUserDataByEmail, [email], function(
          err,
          data
        ) {
          if (data.length > 0) {
            for (index = 0, len = data.length; index < len; index++) {
              for (
                index2 = 0, len2 = excel_data_3.length;
                index2 < len2;
                index2++
              ) {
      ...
      function(excel_data_6, callback) {
        if (excel_data_6.length > 0) {
          var row = null,
            ...
            index = 0,
            write_excel =
              __dirname + "/../public/uploads/excel/regist_employee.xlsx", //쓰기에 사용할 엑셀파일
            output_excel =
              __dirname + "/../public/uploads/excel/fail_upload.xlsx"; //내보내기에 사용할 엑셀파일
...
router.post("/upload/excel/register/employee", isAuthenticated, function(
  req,
  res
) {
  var _file_path = null;
    ...
    [
      ...
      function(callback) {
        connection.query(
          ...
          [_data.group_name, _data.group_desc, req.user.fc_id, _data.group_id],
          function(err, result) {
            if (err) {
              console.error(err);
              callback(err, null);
